...
extends BasicEntityMountLarge {
    public EntityMountMiH(World world) {
        ...
        this.seatPos = new float[]{0.0f, 2.0f, 0.0f};
        this.seatPos2 = new float[]{-0.3f, 0.5f, 0.0f};
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote && this.ticksExisted % 8 == 0) {
            float[] partPos1 = CalcHelper.rotateXZByAxis(1.2f, -0.25f, this.renderYawOffset * ((float)Math.PI / 180), 1.0f);
            ParticleHelper.spawnAttackParticleAt(this.posX + partPos1[1], this.posY + 0.85, this.posZ + partPos1[0], 0.0, 0.0, 0.0, (byte)49);
        }
    }
