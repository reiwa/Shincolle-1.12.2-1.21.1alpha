...
public class EntityBBHaruna extends BasicEntityShipSmall {
    ...
    private void applyBuffToNearbyAllies() {
        ...
        nearbyShips.stream()
                ...
                .forEach(ship -> ship.addPotionEffect(new PotionEffect(MobEffects.LUCK, 100 + this.getStateMinor(0), this.getStateMinor(0) / 120, false, false)));
    }

    @Override
    public void calcShipAttributesAddEffect() {
        super.calcShipAttributesAddEffect();
        this.AttackEffectMap.put(ATTACK_EFFECT_KEY_LUCK_AURA, new int[]{this.getLevel() / 95, 100 + this.getLevel(), this.getLevel()});
    }
