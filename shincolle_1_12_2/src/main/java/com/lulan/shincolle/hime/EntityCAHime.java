...
public class EntityCAHime extends BasicEntityShipSmall {
    ...
    public EntityCAHime(World world) {
        ...
        this.setStateMinor(STATE_MINOR_SHIP_CLASS, 49);
    ...
    public void calcShipAttributesAddRaw() {
        ...
        if (!this.world.isDaytime()) {
            ...
            this.getAttrs().setAttrsRaw(11, this.getAttrs().getAttrsRaw(11) + 0.2f);
        }
    }
    ...
    private void handlePeriodicChecks() {
        if (!this.world.isDaytime() && this.getStateFlag(9)) {
            this.addPotionEffect(new PotionEffect(MobEffects.SPEED, 150, this.getStateMinor(0) / 50, false, false));
            this.addPotionEffect(new PotionEffect(MobEffects.JUMP_BOOST, 150, this.getStateMinor(0) / 40, false, false));
        }

        boolean isTimeForPushCheck = (this.ticksExisted & 0xFF) == 0 && this.getRNG().nextInt(5) == 0;
        boolean canPush = !this.isMorph && !this.isSitting() && !this.isRiding() && !this.getStateFlag(2) && !this.getIsLeashed();

        if (isTimeForPushCheck && canPush) {
            this.findTargetPush();
        }
    }
