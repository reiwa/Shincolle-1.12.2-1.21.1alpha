        ...
        if (!this.world.isRemote && this.ticksExisted % 128 == 0) {
            ...
            if (this.getStateFlag(1) && this.getStateFlag(9) && this.getStateMinor(6) > 50) {
                ...
                for (EntityLivingBase target : hitList) {
                    ...
                    if (target == this || !TeamHelper.checkIsAlly(this, target) || target.getHealth() / target.getMaxHealth() >= 0.94f) continue;
                    if (target instanceof EntityPlayer) {
                        target.heal(4.0f + target.getMaxHealth() * 0.06f + this.getLevel() * 0.06f);
                        canHeal = true;
                    } else if (target instanceof BasicEntityShip) {
                        target.heal(4.0f + target.getMaxHealth() * 0.06f + this.getLevel() * 0.2f);
                        canHeal = true;
                    }
                    if (!canHeal) continue;
